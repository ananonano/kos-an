...
const colorMap = {
  ...
  amber:   { bg: "bg-amber-50 dark:bg-amber-950/30",  icon: "bg-amber-100 dark:bg-amber-900/50 text-amber-600 dark:text-amber-400",  badge: "text-amber-600" },
...
export function StatsCard({ title, value, subtitle, icon: Icon, trend, color = "blue", index = 0 }: StatsCardProps) {
  ...
  return (
    <motion.div
      ...
      className={cn("rounded-xl border p-5 shadow-sm hover:shadow-md transition-shadow", colors.bg)}
    >

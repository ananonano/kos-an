...
export class TenantModel {
    ...
    static async create(data: {
      ...
      `;
            ...
            // If kamar_id is provided, update room status to 'terisi'
            if (data.kamar_id) {
          ...
          WHERE id = $1
        `;
                await client.query(updateRoomQuery, [data.kamar_id]);
            }
    ...
    static async update(id: number, data: {
        ...
        try {
            ...
            // Get current tenant data to check old kamar_id
            const currentTenantQuery = 'SELECT kamar_id FROM tenants WHERE id = $1';
            const currentTenantResult = await client.query(currentTenantQuery, [id]);
            const oldKamarId = currentTenantResult.rows[0]?.kamar_id;
      ...
      `;
            ...
            // Handle room status updates if kamar_id changed
            if (data.kamar_id !== undefined && oldKamarId !== data.kamar_id) {
                // If old room exists, set it to 'kosong'
                if (oldKamarId) {
                    await client.query(`
            UPDATE rooms
          ...
          `, [oldKamarId]);
                ...
                // If new room exists, set it to 'terisi'
                if (data.kamar_id) {
                    await client.query(`
            UPDATE rooms
            SET status = 'terisi', updated_at = NOW()
    ...
    static async delete(id: number): Promise<boolean> {
        ...
        try {
            ...
            // Get tenant's kamar_id before deleting
            const getTenantQuery = 'SELECT kamar_id FROM tenants WHERE id = $1';
            const tenantResult = await client.query(getTenantQuery, [id]);
            ...
            // If tenant had a room, set room status to 'kosong'
            if (kamarId) {
                await client.query(`
          UPDATE rooms
          SET status = 'kosong', updated_at = NOW()
